   ...
   useEffect(() => {
      const fetchProducts = async () => {
         try {
            ...
            // 2. Lọc sản phẩm chỉ lấy danh mục Tinh Dầu (category_id = 5)
            const SCENTS_CATEGORY_ID = 5; // "Tinh Dầu" category ID

            // Lọc sản phẩm theo category_id hoặc categoryId
            let scentProducts = productsData.filter((product: ProductWithPossibleCategories) => {
               return (
                  product.categoryId === SCENTS_CATEGORY_ID ||
                  product.category_id === SCENTS_CATEGORY_ID ||
                  product.category?.id === SCENTS_CATEGORY_ID ||
                  product.categories?.some(cat => cat.id === SCENTS_CATEGORY_ID)
               );
            });

            // Nếu không tìm thấy sản phẩm nào theo ID, lọc theo từ khóa liên quan đến tinh dầu
            ...
            console.log(`Tìm thấy ${scentProducts.length} sản phẩm tinh dầu`);

            // Chuẩn hóa danh sách sản phẩm tinh dầu
            const normalizedProducts = scentProducts.map((product) => ({
               ...product,
               images: Array.isArray(product.images) ? product.images : [product.images],
            }));

            // 3. Lấy chi tiết sản phẩm và giá cho từng sản phẩm tinh dầu
