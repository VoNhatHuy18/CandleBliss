...
export default function AddressPage() {
   ...
   const getUserNameAndPhone = useCallback(async (userId: number) => {
      try {
         const token = localStorage.getItem('token');
         ...
         try {
            ...
            const addressPromises = userAddressIds.map((id: number) =>
               fetch(`${HOST}/api/v1/address/${id}`, {
                  ...
                  .then((response) => {
                     if (response.ok) {
                        return response.json() as Promise<ApiAddress>;
                     }
                     return null;
                  })
                  .catch(() => null),
   ...
   const updateAddress = async (addressData: {
      ...
      try {
         ...
         if (!response.ok) {
            throw new Error(
               `Failed to ${isUpdate ? 'update' : 'create'} address: ${response.status}`,
            );
         }
   ...
   const handleSaveAddress = async (e: React.FormEvent) => {
      ...
      try {
         ...
         if (newAddress.id) {
            setAddresses((prev) =>
               prev.map((addr) => (addr.id === formattedAddress.id ? formattedAddress : addr)),
            );
                     ...
                     {isLoading ? (
                        ...
                        <form onSubmit={handleSaveAddress} className='space-y-4'>
                           ...
                           <div>
                              ...
                              <select
                                 name='province'
                                 value={
                                    provinces.find((p) => p.name === newAddress.province)?.id || ''
                                 }
                           ...
                           <div>
                              ...
                              <select
                                 name='district'
                                 value={
                                    districts.find((d) => d.name === newAddress.district)?.id || ''
                                 }
                        ...
                        <div className='space-y-4'>
                           {addresses.map((address) => (
                              <div
                                 key={address.id}
                                 className='border rounded-lg p-4 relative hover:border-amber-300 transition-colors'
                              >
                                 {address.isDefault && (
         ...
         {showDeleteModal && currentAddress && (
            <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
               <div className='bg-white rounded-lg p-6 max-w-sm mx-4'>
                  ...
                  <p className='text-gray-600 mb-5'>Bạn có chắc chắn muốn xóa địa chỉ này?</p>
